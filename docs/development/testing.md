...
OpenClaw 使用 **Vitest**（v4.1+）+ V8 覆盖率，最低覆盖率要求 70%。测试分三类文件：

| 后缀 | 类型 |
|---|---|
| `*.test.ts` | 单元 / 集成 |
| `*.e2e.test.ts` | Gateway 端到端烟雾测试 |
| `*.live.test.ts` | 真实 Provider 测试（需 `LIVE=1` + 真实 API Key） |

Live 测试的最高层是**三个 I/O 探针**：让 Agent 读文件、写文件、识别图片——只验证「有没有执行」，不验证「行为对不对」。

**官方承认的盲区**（来自官方文档）：

- **决策测试**：给 Agent 一个输入，它选了哪个 Skill？选对了吗？
- **技能合规检查**：Agent 有没有读 `SKILL.md` 并按要求执行所有步骤？
- **多轮工作流契约**：工具调用顺序、session 历史传递、沙箱边界是否正确？

官方计划用「mock-provider 场景运行器」填补，但尚未实现。第三方包 [`claw-harness`](https://www.npmjs.com/package/claw-harness)（v0.3.0）在往这个方向补，用 YAML 定义多 Agent 场景，但仍处于早期。

**结论**：官方测试套件覆盖底层基础设施，但对「Agent 收到输入后如何决策、如何干活」完全没有系统性断言。本文档的设计就是为了填补这一空白。
...
| E | [`claw-harness`](https://www.npmjs.com/package/claw-harness)（YAML 场景 DSL） | 解释器已存在、无需自建 | v0.3.0 早期、外部依赖、场景语法固定 |

**推荐**：A（剧本 LLM）作为 Layer 2-3 主方案，C（直接调用）作为 Layer 1。方案 E 可在方案 A 实现复杂度过高时作为 fallback。
...
1. **OpenClaw LLM API 格式**：官方测试套件使用 `ANTHROPIC_API_KEY`，推测为 Anthropic SDK 格式（`tool_use` block 结构）。需在 VM 内确认，特别是多 provider 场景下是否统一。
   ```bash
   # 在 VM 内抓包或查看日志
...
3. **Agent 无头消息注入**：`openclaw agent --message` 是否存在？`claw-harness` 的实现是启动独立 gateway 进程（绑定指定端口）后通过 HTTP 发消息，而非直接调用 CLI——实际注入方式可能是 HTTP API 而非命令行参数。
   ```bash
   openclaw agent --help
