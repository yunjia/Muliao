# Plan: RPi 5 部署方案 — Pantheon 极简 AI 助手设备

## TL;DR

将 Pantheon 打包为**预装好的 RPi 5 成品设备**，用户拿到手后插电联网即可使用。底层仍是 Docker，用户通过 **`pantheon.ai` 云端 PWA** 完成设备初始化和日常管理——手机浏览器访问 `pantheon.ai`，PWA 通过云端中继与局域网内的 RPi 设备通信，将所有 IT 概念对用户完全隐藏。分两个大阶段推进：**Phase A（团队辅助部署）** → **Phase B（DIY 开箱即用）**。
...
用户触点：
  ...
  - pantheon.ai PWA ←→ RPi（通过云端 Relay 中转）
```
...
### 核心交互界面：pantheon.ai 云端 PWA

所有目标客户都是手机用户。用户访问 **`pantheon.ai`**（云端托管的 PWA），通过云端中继与局域网内的 RPi 设备通信。可"添加到主屏幕"获得 App 级体验。

**为什么是云端 PWA（`pantheon.ai`）而非本地 Web（`pantheon.local`）：**
- 用户只需记住一个正常网址 `pantheon.ai`，无需理解局域网地址
...
**设备发现与通信机制：**
- RPi 启动后，Device Agent 主动通过 WebSocket 外连 `pantheon.ai` 云端 Relay
- 用户在 `pantheon.ai` 登录后，PWA 通过 Relay 与自己绑定的设备通信
...
**Phase A 方案（团队辅助 / pantheon.ai 引导）：**
- 由团队成员在设备出厂前完成 API Key 配置；或通过 pantheon.ai PWA 远程引导用户完成
...
**在 `pantheon.ai` 架构下，这个问题基本消失：**
- RPi 启动后 Device Agent 主动连接 `pantheon.ai` 云端
- 用户在 `pantheon.ai` 上看到设备自动上线，无需手动输入任何地址
- 设备发现 = 后端匹配（Device Agent 上报 → 云端关联到用户账号 → PWA 显示设备在线）

**配对方式（首次绑定设备到用户账号）：**
- **Phase A**：团队出厂前预绑定，用户无感
- **Phase B**：设备附带配对码（印在说明卡片上 / OLED 屏显示），用户在 `pantheon.ai` 输入配对码完成绑定

**离线 fallback（可选）：**
- 保留 Avahi mDNS，设备仍可通过 `pantheon.local` 局域网直连（备用）
- OLED 小屏显示 IP + 配对码（后期加分项）

### 难题 4: 首次联网（WiFi 配网）

无屏设备的经典鸡生蛋问题：**设备还没联网 → 没法访问 pantheon.ai → 没法配置 WiFi**。
...
RPi 首次启动，检测到无网络连接
  │
  ├─ 自动创建 WiFi 热点："Pantheon-XXXX"（XXXX = 配对码后 4 位）
  ...
  │   │   └─ 页面提示："连接成功！请切回家庭 WiFi，打开 pantheon.ai 继续"
  │   └─ 失败 → 提示"密码可能不对，请重试"
  │
...
- AP ↔ Station 模式切换由 `pantheon-wifi-setup.service` 管理
...
Step 1: 烧录系统镜像
  ...
  └─ 写入 cloud-init user-data，首次启动自动安装 Docker、拉取 Pantheon 镜像、启用 systemd services

Step 2: 首次启动 & 基础配置
  └─ 接上网络，设备自动连接 pantheon.ai 云端
  └─ 通过 pantheon.ai PWA 完成配置（或 SSH 辅助）：
      - 配对设备到用户账号
...
用户拿到设备
  ...
  ├─ 1.5 连接网络
  │     ├─ 有网线？插上即可，跳到步骤 2
  │     └─ 纯 WiFi？手机连接 "Pantheon-XXXX" 热点
  │           └─ 弹出配网页面 → 选择家庭 WiFi → 输密码 → 连接成功
  │           └─ 手机切回家庭 WiFi
  │
  ├─ 2. 手机浏览器访问 pantheon.ai（扫说明卡上的二维码，或直接输入网址）
...
- `device-agent` — 设备代理（维持与 `pantheon.ai` 云端 Relay 的 WebSocket 长连接，转发配置指令到本地）
...
runcmd:
  # 预拉取 Docker 镜像
  - docker pull ghcr.io/teabots/pantheon:latest
  - docker pull containrrr/watchtower
  # 启用 Pantheon 服务
  - systemctl enable --now pantheon.service

write_files:
  - path: /etc/systemd/system/pantheon.service
...
- `deploy/rpi/pantheon.service` — systemd unit file
...
**设置流程**：用户在 Web 门户选择场景后，`pantheon-setup` 脚本将对应模板复制到 `teams/<name>/workspace/` 并应用 config overlay。

### Step 4: pantheon.ai 云端 PWA + Device Agent
*依赖：Step 1（Compose 结构），Step 3（模板）*

这是整个用户体验的核心入口。拆分为两个子项目：

#### Step 4a: pantheon.ai 云端服务（独立部署）
...
- 启动时通过 outbound WebSocket 连接 `pantheon.ai` 云端 Relay
...
- 通信：WebSocket client → `wss://relay.pantheon.ai`
...
- Watchtower 容器定期检查 `ghcr.io/teabots/pantheon:latest` 是否有新镜像
...
| 状态监控 | pantheon.ai 仪表盘 | pantheon.ai 仪表盘 |
| 备份恢复 | CLI backup.sh + PWA | PWA 操作 |
| 多用户/团队 | ❌ 单用户 | ✅ pantheon.ai 管理多团队 |
...
| `deploy/rpi/pantheon.service` | systemd 开机自启服务 |
...
| `pantheon-cloud/`（独立仓库） | pantheon.ai 云端服务：PWA 前端 + Relay 后端 + 用户账号 |
...
| 用户入口 | `pantheon.ai` 云端 PWA（手机优先） | 正常网址易记、HTTPS 标配、不受 mDNS 兼容性限制、出门也能管理设备 |
...
4. **pantheon.ai 云端成本**：Relay 服务 + 静态托管 + 用户账号存储，初期免费 tier 方案？Cloudflare Pages (免费静态托管) + Fly.io (免费 tier 小实例) 能否支撑早期用户量？
